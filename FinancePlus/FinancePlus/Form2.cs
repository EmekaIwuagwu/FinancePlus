    ...
    public partial class BalanceSheet : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                dtb3 = GetBalanceStatement(dtb3);
                reportViewer1.LocalReport.DataSources.Clear();
                ReportDataSource rpd = new ReportDataSource("DataSet1", dtb);
                ReportDataSource rpd2 = new ReportDataSource("DataSet2", dtb2);
                ReportDataSource rpd3 = new ReportDataSource("DataSet3", dtb3);
                reportViewer1.LocalReport.DataSources.Add(rpd);
                reportViewer1.LocalReport.DataSources.Add(rpd2);
                reportViewer1.LocalReport.DataSources.Add(rpd3);
                reportViewer1.RefreshReport();
            }
        }

        private DataTable GetBalanceStatement(DataTable dt)
        {
            using (SqlConnection cn = new SqlConnection(constring))
            {
                try
                {
                    SqlDataAdapter da = new SqlDataAdapter("select opening_amount as [opening_amount] from account_info where accountNo = '" + accountNo1.Text + "'", cn);
                    da.Fill(dt);
                }
                catch(Exception ex)
                {
